/*
 * Notes for HealthyPi Clinical
 * 
 * 1) Remember tp update HRV_CHARACTERISTIC_UUID and HIST_CHARACTERISTIC_UUID
 *    values in flutter app.
 *  2) Resolve differences in MyCallbackHandler
 *  3) hrv_array[10:12] => hrv_array[8:10]
 */
...
/*
 * HealthyPi Clinical Firmware Extensions
 * 
 * 1) Permit multiple connections.  HealthyPi remains in connected state as long as at least one device
 *    remains connected
 * 2) New broadcast/ beacon mode where measurements are reported on a regular basis, even if no devices
 *    are connected
 *    a) Change in format of advertising data to include measurement data and to limit advertising of
 *       characteristic information to avoid field overflow
 *    b) Continue to collection and process measurement data even if no device is connected
 * 3) New BLE Characteristics:
 *    a) Patiebt name/identifier. R/W/Notify field with broadcast of hash to help identify when device
 *       has been reassigned (HP_NAME_CHARACTERISTIC_UUID)
 *    b) Consolidated vitals characteristic (r/n) returning HR, RR, SPO2 and Temp
 *      (HP_VITALS_CHARACTERISTIC_UUID)
 */


#define DO_BROADCAST_VITALS TRUE      // Enables broadcast mode for HealthyPi Clinical
#define BLE_DEVICE_NAME "Healthypi v4"
...
#include <BLEUtils.h>
#include <BLE2902.h>
#include <BLEAdvertising.h>      // clinical
...
// Clinical - Repurpose HCR_Print Characteristic / HCR_Scan Characteristic 
...
BLECharacteristic* name_Characteristic = NULL;     // Clinical
BLECharacteristic* vitals_Characteristic = NULL;   // Clinical
...
//
// New globals and struct for HealthyPi Clinical
//

const int man_code = 0x02E5;   //manufacturer code (0x02E5 for Espressif)
char mfg_update_seq = 0;
uint vital_count = 0;                       // used to determine when to notify vitals change
int deviceConnectedCount = 0;               // track multiple connections
String nameValue = "";                      // patient identifier
unsigned int nameHash = 0;                  // hash of patient identifier (used to identify reassignment)
uint8_t last_sp02 = 0;                      // stable copy of sp02

struct vitals_notification {                       // clinical
  uint16_t temp;
  uint8_t hr;
  uint8_t rr;
  uint8_t spo2;
...
class MyServerCallbacks : public BLEServerCallbacks
{
  void onConnect(BLEServer *pServer)
  {
    ...
    deviceConnectedCount++;                 // clinical
    Serial.println("connected");

    // Keep advertising
    BLEAdvertising *pAdvertising = BLEDevice::getAdvertising();  // clinical
    pAdvertising->start();                                       // clinical
  }

  void onDisconnect(BLEServer *pServer)
  {
    deviceConnectedCount--;                        // clinical
    if (deviceConnectedCount == 0) {               // clinical
      deviceConnected = false;
      Serial.println("all disconnected");          // clinical
    } else {                                       // clinical

      Serial.println("device disconnected");       // clinical
    }
  }
};


// Note: Appears to be experimental code to demp write ops
...
/*
 * Code related to WiFi and Web Interface
 */
...
{
  BLEDevice::init(BLE_DEVICE_NAME); // Create the BLE Device -- Clinical
  ...
  name_Characteristic->addDescriptor(new BLE2902());               // clinical
  name_Characteristic->setCallbacks(new NameCallbackHandler());    // clinical
  vitals_Characteristic->addDescriptor(new BLE2902());             // clinical
...
{
  long adv_count=0;                                   // Clinical
  const long blink_off = (100*SAMPLING_RATE/1000);    // Clinical
  const long interval = (3000*SAMPLING_RATE/1000);    // Clinical

  if (DO_BROADCAST_VITALS) {
    // Continue to broadcast while waiting for connection
...
int toggle = 0;         // Clinical

// New for clinical
void update_advertising() {
  // Updates information in advertising packet when in vitals broadcast mode
...
void handle_ble_stack()
{
  ...
  if (spo2_calc_done)
  {
    ...
    last_sp02 = (uint8_t)sp02;  // Clinical
    Serial.printf("SpO2 %d %d\n", (uint8_t)sp02, (uint8_t)(sp02 >> 8));
  }
...
/*
 * Top-level initialization -- Initialize chips and sensors
 */ 
...
void setup()
{
  ...
  if(Healthypi_Mode == BLE_MODE)  // Added for clinical
  {
    ble_advertising();
    Serial.println("Waiting a client connection to notify...");
  }
...
boolean get_ADS1292R_data() {
  ...
  else
  {
    ...
    // RR Calculation using FFT above
...
boolean processSensors()
{
  ...
  if ((time_count++ * (1000 / SAMPLING_RATE)) > MAX30205_READ_INTERVAL)
  {
    ...
    read_battery_value();
    if (DO_BROADCAST_VITALS)    // Clinical
        update_advertising();                 // broadcast updated values
  }

  if (vital_count++ == SAMPLING_RATE<<1) {    // Clinical
    vital_count = 0;
    update_vitals();                          // send notification to vitals subscribers
  }
...
// Somplified loop() -- see processSensors(), processSwitches() and processWiFi() for refactord content
void loop()
{
  boolean has_new_data = processSensors();
  if (has_new_data) {
    if (Healthypi_Mode == BLE_MODE)
    {
      handle_ble_stack();
